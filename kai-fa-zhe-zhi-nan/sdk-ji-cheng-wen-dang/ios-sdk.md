...
* 请将以下代码添加到 - (BOOL)application:(UIApplication _)application didFinishLaunchingWithOptions:(NSDictionary _)launchOptions
...
* 在收到 deviceToken 的系统回调方法 - (void)application:(UIApplication _)application didRegisterForRemoteNotificationsWithDeviceToken:(NSData _)deviceToken 中上报 deviceToken
...
* 若实现了 Notification Service Extension 扩展，当收到推送时，除了会执行扩展的回调方法，还会执行以上收到推送的回调方法，为避免重复上报，需要将上述上报 PUSH_RECEIVE 类型的事件注释掉，或者根据项目需要采用宏定义判断的方式来规避掉重复上报。
...
* (void)pushToken:(NSData _)deviceToken groupIdentifier:(NSString _)groupIdentifier;
...
* (void)pushTrack:(PushEventType)type msg:(NSDictionary _)msg groupIdentifier:(NSString _)groupIdentifier;
...
* (void)setWeChatId:(NSString _)unionId appId:(NSString _)appid openId:(NSString \*)openId;
